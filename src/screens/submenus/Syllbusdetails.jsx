...
const Syllabusdetails = () => {
  ...
  const [syllbusdetailsData, setsyllbusdetailsData] = useState([]);
  const [errors, setErrors] = useState("");
  const navigate = useNavigate();

  useEffect(() => {
    fetchsyllbusdetailsData();
  ...
  }, [syllbusdetailsData, filteredData]);




  const fetchsyllbusdetailsData = async () => {
    setLoading(true);
    try {
      ...
      const response = await axios.get(`${BASE_URL}/get_all_syllabus`, {
        headers: {
      ...
      if (response.data?.status === "Success") {
        ...
        toast.error("Failed to fetch syllabus details data");
      }
    } catch (err) {
      console.error("Error fetching syllabus details data:", err);
      toast.error("Error fetching syllabus details data. Please check the console.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const tableColumns = (currentPage, rowsPerPage) => [
    ...
    {
      name: "Module Name",
      selector: (row) => row.module_name || "N/A",
    },
    {
      name: "Title ",
      ...
      cell: (row) => (
        <div className="d-flex">
          <OverlayTrigger placement="top" overlay={<Tooltip id="edit-tooltip">Edit</Tooltip>}>
            <Button className="ms-1" onClick={() => navigate(`/update-syllabus/${row.id}`, { state: row })}>
              <FaEdit />
            </Button>
          </OverlayTrigger>
            ...
            <Card.Header>
              <Row>
                <Col className="d-flex align-items-center">
                  <h5>Syllabus <span className="highlight"> List</span></h5>
                </Col>
              ...
              <Row className="mt-3">
                <Col className="d-flex justify-content-end">
                  <Button variant="primary" onClick={handleAddSyllabus}>
                    <FaPlus /> Add Syllabus
                  </Button>
                </Col>
              </Row>
