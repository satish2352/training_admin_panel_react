...
const Syllabuspdfdetails = () => {
  ...
  const [syllabuspdfdetails, setsyllabuspdfdetails] = useState([]);
  const [errors, setErrors] = useState("");
  const navigate = useNavigate();

  useEffect(() => {
    fetchsyllabuspdfdetails();
  ...
  }, [syllabuspdfdetails, filteredData]);
  ...
  const fetchsyllabuspdfdetails = async () => {
    setLoading(true);
    try {
      ...
      const response = await axios.post(`${BASE_URL}/getAllDataList`, {
        headers: {
      ...
      setsyllabuspdfdetails(sortedData); // Set sorted data
      setData(sortedData); // Update the SearchExportContext data


      if (response.data?.status === "Success") {
        setsyllabuspdfdetails(response.data.data);
      } else {
        console.error("Unexpected API response structure:", response.data);
        toast.error("Failed to fetch syllabus pdf data");
      }
    } catch (err) {
      console.error("Error fetching syllabus pdf data:", err);
      toast.error("Error fetching syllabus pdf data. Please check the console.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddSyllabuspdf = () => {
    navigate("/addsyllabuspdf");
  };
      ...
      cell: (row) => (
        <div className="d-flex">
          <OverlayTrigger placement="top" overlay={<Tooltip id="edit-tooltip">Edit</Tooltip>}>
            <Button className="ms-1" onClick={() => navigate(`/update-syllabuspdf/${row.id}`, { state: row })}>
              <FaEdit />
            </Button>
          </OverlayTrigger>
            ...
            <Card.Header>
              <Row>
                <Col className="d-flex align-items-center">
                  <h5>Syllabus Pdf <span className="highlight"> List</span></h5>
                </Col>
              ...
              <Row className="mt-3">
                <Col className="d-flex justify-content-end">
                  <Button variant="primary" onClick={handleAddSyllabuspdf}>
                    <FaPlus /> Add Syllabus Pdf
                  </Button>
                </Col>
              </Row>
            </Card.Header>
...
export default Syllabuspdfdetails;
