...
useEffect(() => {
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "View T3");
    XLSX.writeFile(workbook, "View-T3.xlsx");
  };
      ...
      cell: (row) => (
        <div className="d-flex">
          <OverlayTrigger placement="top" overlay={<Tooltip id="view-tooltip">View</Tooltip>}>
            <Button className="ms-1" variant="secondary" onClick={() => navigate(`/no-due-clearance/${row.id}`)}>
              <FaEye />
            </Button>
          </OverlayTrigger>
...
export default NoClearanceDueList;