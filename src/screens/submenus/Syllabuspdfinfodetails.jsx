...
const Syllabuspdfinfodetails = () => {
  ...
  const [syllabuspdfinfoData, setsyllabuspdfinfoData] = useState([]);
  const [errors, setErrors] = useState("");
  const navigate = useNavigate();

  useEffect(() => {
    fetchsyllabuspdfinfoData();
  ...
  }, [syllabuspdfinfoData, filteredData]);
  ...
  const fetchsyllabuspdfinfoData = async () => {
    setLoading(true);
    try {
      ...
      const response = await axios.get(`${BASE_URL}/get_brochuer`, {
        headers: {
          ...
          <Card>
            <Card.Header>
              <Row>
                <Col className="d-flex align-items-center">
                  <h5>Syllabus pdf info <span className="highlight"> List</span></h5>
                </Col>
            ...
            <Card.Body>
              <DataTable
                ...
                data={searchQuery ? filteredData : syllabuspdfinfoData}
                pagination
...
export default Syllabuspdfinfodetails;
