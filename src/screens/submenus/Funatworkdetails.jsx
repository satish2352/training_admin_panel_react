...
const Funatworkdetails = () => {
  ...
  const [funatworkData, setfunatworkData] = useState([]);
  ...
  const fetchfunatworkData = async () => {
    setLoading(true);
    try {
      ...
      const response = await axios.get(`${BASE_URL}/get_funatworkdetails`, {
        headers: {
      ...
      setfunatworkData(sortedData); // Set sorted data
      setData(sortedData); // Update the SearchExportContext data
      console.log("API Response:", response.data); // Debugging log

      if (response.data?.status === "Success") {
        setfunatworkData(response.data);
      } else {
      }
    } catch (err) {
      console.error("Error fetching fun at work data:", err);
      toast.error("Error fetching fun at work data. Please check the console.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "20px" }}>
            <button
              ...
              onClick={async () => {
                ...
                try {
                  await instance.delete(`delete_funatworkdetails/${id}`, {
                    headers: {
                  ...
                  setfunatworkData((prevCourses) => prevCourses.filter(course => course.id !== id));
      ...
      cell: (row) => (
        <div className="d-flex">
          <OverlayTrigger placement="top" overlay={<Tooltip id="edit-tooltip">Edit</Tooltip>}>
            <Button className="ms-1" onClick={() => navigate(`/update-funatworkdata/${row.id}`, { state: row })}>
              <FaEdit />
            </Button>
          </OverlayTrigger>
            ...
            <Card.Header>
              <Row>
                <Col className="d-flex align-items-center">
                  <h5>Fun Activity   <span className="highlight"> List</span></h5>
                </Col>
              ...
              <Row className="mt-3">
                <Col className="d-flex justify-content-end">
                  <Button variant="primary" onClick={handleAddFunatworkdata}>
                    <FaPlus /> Add Fun at work Data
                  </Button>
                </Col>
              </Row>
            </Card.Header>
...
export default Funatworkdetails;
